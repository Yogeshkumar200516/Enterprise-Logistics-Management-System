// src/routes/supervisorHistoryRoutes.js
...
// so MySQL can resolve it unambiguously.
...
// ─────────────────────────────────────────────────────────────────────────────
// GET /api/supervisor-history/deliveries
// ─────────────────────────────────────────────────────────────────────────────
router.get("/deliveries", async (req, res) => {
  try {
    const supervisorId = req.user.user_id;
    const tenantId     = req.user.tenant_id;
    const { period, startDate, endDate, status, driver_id, page = 1, limit = 15 } = req.query;

    // Use fully-qualified alias so it's unambiguous in the JOIN context
    ...
    const [[{ total }]] = await pool.query(`
      ...
      WHERE da.supervisor_id = ? AND da.tenant_id = ? ${df.clause} ${extraWhere}
    `, [supervisorId, tenantId, ...df.params, ...extraParams]);
    ...
    const [rows] = await pool.query(`
      ...
      LIMIT ? OFFSET ?
    `, [supervisorId, tenantId, ...df.params, ...extraParams, parseInt(limit), offset]);

    res.json({ success: true, data: rows, total: Number(total), page: parseInt(page), limit: parseInt(limit) });
  } catch (err) {
    console.error("[supervisor-history/deliveries]", err);
    res.status(500).json({ success: false, message: err.message });
  }
});

// ─────────────────────────────────────────────────────────────────────────────
// GET /api/supervisor-history/scraps
// FIX: pass "sl.created_at" — plain "created_at" is ambiguous because
//      scrap_items also has a created_at column and we LEFT JOIN it.
// ─────────────────────────────────────────────────────────────────────────────
router.get("/scraps", async (req, res) => {
  try {
    const supervisorId = req.user.user_id;
    const tenantId     = req.user.tenant_id;
    const { period, startDate, endDate, status, source, driver_id, page = 1, limit = 15 } = req.query;

    // ✅ Fully-qualified alias prevents "Column 'created_at' is ambiguous"
    ...
    const [[{ total }]] = await pool.query(`
      SELECT COUNT(*) AS total
      FROM scrap_logs sl
      WHERE sl.supervisor_id = ? AND sl.tenant_id = ? ${df.clause} ${extraWhere}
    `, [supervisorId, tenantId, ...df.params, ...extraParams]);
    ...
    const [rows] = await pool.query(`
      ...
      GROUP BY
        ...
        dr.full_name, v.vehicle_number, v.vehicle_type
    ...
    console.error("[supervisor-history/scraps]", err);
    res.status(500).json({ success: false, message: err.message });
  }
});

// ─────────────────────────────────────────────────────────────────────────────
// GET /api/supervisor-history/orders
// ─────────────────────────────────────────────────────────────────────────────
router.get("/orders", async (req, res) => {
  try {
    ...
    // orders JOIN order_items — use "o.created_at" to be safe
    ...
    console.error("[supervisor-history/orders]", err);
    res.status(500).json({ success: false, message: err.message });
  }
});

// ─────────────────────────────────────────────────────────────────────────────
// GET /api/supervisor-history/filters
// ─────────────────────────────────────────────────────────────────────────────
router.get("/filters", async (req, res) => {
  try {
    ...
    console.error("[supervisor-history/filters]", err);
    res.status(500).json({ success: false, message: err.message });
  }
});

// ─────────────────────────────────────────────────────────────────────────────
// GET /api/supervisor-history/delivery/:id
// ─────────────────────────────────────────────────────────────────────────────
router.get("/delivery/:id", async (req, res) => {
  try {
    ...
    const [[delivery]] = await pool.query(`
      ...
      JOIN users    dr ON dr.user_id   = da.driver_id
      JOIN vehicles v  ON v.vehicle_id = da.vehicle_id
      WHERE da.delivery_id = ? AND da.supervisor_id = ? AND da.tenant_id = ?
    `, [deliveryId, supervisorId, tenantId]);
    ...
    console.error("[supervisor-history/delivery/:id]", err);
    res.status(500).json({ success: false, message: err.message });
  }
});

// ─────────────────────────────────────────────────────────────────────────────
// GET /api/supervisor-history/scrap/:id
// ─────────────────────────────────────────────────────────────────────────────
router.get("/scrap/:id", async (req, res) => {
  try {
    ...
    const [[scrap]] = await pool.query(`
      ...
      JOIN users    dr ON dr.user_id   = sl.driver_id
      JOIN vehicles v  ON v.vehicle_id = sl.vehicle_id
      WHERE sl.scrap_id = ? AND sl.supervisor_id = ? AND sl.tenant_id = ?
    `, [scrapId, supervisorId, tenantId]);
    ...
    console.error("[supervisor-history/scrap/:id]", err);
    res.status(500).json({ success: false, message: err.message });
  }
});
