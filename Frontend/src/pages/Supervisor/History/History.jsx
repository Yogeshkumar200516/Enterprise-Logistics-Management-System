// src/pages/SupervisorHistory.jsx
...
import {
  ...
  Person, DirectionsCar, SupervisorAccount,
  CalendarMonth, Place,
...
const ORDER_STATUS_COLORS = {
  NOT_ASSIGNED: "#888", IN_PROGRESS: "#ff9800", DELIVERED: "#4caf50",
};
...
function DetailDrawer({ open, onClose, type, id }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setDetail(null);
    api.get(`/api/supervisor-history/${type}/${id}`)
  ...
  return (
    <Drawer anchor="right" open={open} onClose={onClose}
      PaperProps={{ sx: { width: { xs: "100%", sm: 500 }, background: isDark ? "#0f2027" : "#f4f8ff", borderLeft: `1px solid ${isDark ? alpha("#fff", 0.08) : alpha("#024990", 0.1)}` } }}>
              ...
              <Paper key={idx} elevation={0} sx={{ p: 1.5, mb: 1, borderRadius: 2, border: `1px solid ${isDark ? alpha("#fff", 0.06) : alpha("#024990", 0.08)}`, background: isDark ? alpha("#fff", 0.02) : alpha("#024990", 0.02) }}>
                {isDelivery ? (
                  <>
                    ...
                    <Typography variant="caption" color="text.secondary">Qty: {item.quantity} · {item.is_fragile ? "⚠ Fragile" : "Standard"} · {item.order_reference}</Typography>
                    <Typography variant="caption" color="text.secondary" sx={{ display: "block" }}>{item.customer_name} — {item.customer_address}</Typography>
                  </>
                ) : (
                  <>
                    <Box sx={{ display: "flex", justifyContent: "space-between", mb: 0.5 }}>
...
export default function SupervisorHistory() {
  ...
  const [filters, setFilters] = useState({ period: "month", status: "", driver_id: "", source: "", page: 1, limit: 15 });

  useEffect(() => {
    api.get("/api/supervisor-history/filters")
      .then((r) => setDrivers(r.data.data.drivers || []))
      .catch(() => {});
  }, []);

  const endpoints = ["/api/supervisor-history/deliveries", "/api/supervisor-history/scraps", "/api/supervisor-history/orders"];
  ...
  const handleTabChange = (_, v) => {
    setTab(v);
    setFilters((f) => ({ ...f, status: "", source: "", driver_id: "", page: 1 }));
    setData([]);
  };
    ...
    <Box sx={{ p: { xs: 2, md: 3 }, minHeight: "100vh", bgcolor: "background.default" }}>
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <Box sx={{
          ...
          {[200, 130, 75].map((s, i) => (
            <Box key={i} sx={{ position: "absolute", right: 55 * i - 15, top: -s / 3, width: s, height: s, borderRadius: "50%", bgcolor: alpha("#22fbff", 0.07 - i * 0.015), pointerEvents: "none" }} />
          ))}
          <Box sx={{ position: "relative", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 2 }}>
            <Box>
              <Typography variant="h4" sx={{ color: "#fff", fontWeight: 800, lineHeight: 1.1 }}>History</Typography>
              <Typography variant="body2" sx={{ color: "#22fbff", mt: 0.5, fontWeight: 500 }}>Your deliveries, scrap runs and orders</Typography>
            </Box>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              <Chip label={`${total} records`} sx={{ bgcolor: alpha("#fff", 0.15), color: "#fff", fontWeight: 700 }} />
          ...
          <Select value={filters.status} label="Status" onChange={(e) => setFilters((f) => ({ ...f, status: e.target.value, page: 1 }))}>
            ...
            {(tab === 0
              ...
              : tab === 1
                ...
                : ["NOT_ASSIGNED","IN_PROGRESS","DELIVERED"]
          ...
          <>
            <TableContainer>
              <Table size="small">
                <TableHead>
                  <TableRow>
                    {tab === 0 && [hdr("ID"), hdr("Driver"), hdr("Vehicle"), hdr("Progress"), hdr("Damaged"), hdr("Status"), hdr("Date"), hdr("View")]}
                    {tab === 1 && [hdr("ID"), hdr("Driver"), hdr("Type"), hdr("Source"), hdr("Vehicle"), hdr("Items"), hdr("Status"), hdr("Date"), hdr("View")]}
                ...
                <TableBody>
                  {loading
                    ? Array.from({ length: 8 }).map((_, i) => (
                        <TableRow key={i}>
                          {Array.from({ length: tab === 2 ? 8 : 9 }).map((_, j) => <TableCell key={j}><Skeleton height={24} /></TableCell>)}
                        </TableRow>
                      ))
                    : data.length === 0
                      ? <TableRow><TableCell colSpan={10} align="center" sx={{ py: 6 }}><Typography color="text.secondary" variant="body2">No records found</Typography></TableCell></TableRow>
                      : data.map((row) => {
                              ...
                              <TableRow key={row.delivery_id} sx={hover}>
                                ...
                                <TableCell sx={{ minWidth: 110 }}>
                                  <Box sx={{ display: "flex", alignItems: "center", gap: 0.8 }}>
                                    <LinearProgress variant="determinate" value={pct} sx={{ flex: 1, height: 5, borderRadius: 3, bgcolor: isDark ? alpha("#fff", 0.08) : alpha("#024990", 0.1), "& .MuiLinearProgress-bar": { bgcolor: "#4caf50", borderRadius: 3 } }} />
                                    <Typography variant="caption" color="text.secondary">{row.delivered_items}/{row.total_items}</Typography>
                                  </Box>
                                </TableCell>
                                <TableCell>{row.damaged_items > 0 ? <Chip label={row.damaged_items} size="small" sx={{ bgcolor: alpha("#f44336", 0.15), color: "#f44336", fontWeight: 700, fontSize: 10 }} /> : <Typography variant="caption" color="text.disabled">—</Typography>}</TableCell>
                                <TableCell><StatusChip status={row.status} colorMap={DELIVERY_STATUS_COLORS} /></TableCell>
                                <TableCell sx={{ fontSize: 11, color: "text.secondary", whiteSpace: "nowrap" }}>{new Date(row.assigned_at).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "2-digit" })}</TableCell>
                                <TableCell>
                                  <Tooltip title="View Details"><IconButton size="small" onClick={() => setDrawer({ open: true, type: "delivery", id: row.delivery_id })} sx={{ color: isDark ? "#91eff1" : "#024990" }}><Visibility fontSize="small" /></IconButton></Tooltip>
                                </TableCell>
                              </TableRow>
                            );
                          }
                          if (tab === 1) {
                            return (
                              <TableRow key={row.scrap_id} sx={hover}>
                                ...
                                <TableCell><Chip label={row.source} size="small" sx={{ bgcolor: alpha(row.source === "INTERNAL" ? "#0891b2" : "#9c27b0", 0.15), color: row.source === "INTERNAL" ? "#0891b2" : "#9c27b0", fontWeight: 600, fontSize: 10 }} /></TableCell>
                                <TableCell sx={{ fontSize: 12, color: "text.secondary" }}>{row.vehicle_number}</TableCell>
                                ...
                                <TableCell sx={{ fontSize: 11, color: "text.secondary", whiteSpace: "nowrap" }}>{new Date(row.created_at).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "2-digit" })}</TableCell>
                                <TableCell>
                                  <Tooltip title="View Details"><IconButton size="small" onClick={() => setDrawer({ open: true, type: "scrap", id: row.scrap_id })} sx={{ color: isDark ? "#91eff1" : "#024990" }}><Visibility fontSize="small" /></IconButton></Tooltip>
                                </TableCell>
                              </TableRow>
                            );
                          }
                          return (
                            <TableRow key={row.order_id} sx={hover}>
                              ...
                              <TableCell sx={{ fontSize: 11, color: "text.secondary", maxWidth: 160 }}><Typography variant="caption" noWrap>{row.customer_address}</Typography></TableCell>
                              <TableCell sx={{ fontSize: 12 }}>{row.total_items}</TableCell>
                              ...
                              <TableCell sx={{ fontSize: 11, color: "text.secondary", whiteSpace: "nowrap" }}>{new Date(row.created_at).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "2-digit" })}</TableCell>
                              <TableCell sx={{ fontSize: 11, color: "text.secondary", whiteSpace: "nowrap" }}>{row.delivered_at ? new Date(row.delivered_at).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "2-digit" }) : "—"}</TableCell>
                            </TableRow>
                          );
                        })
                  }
